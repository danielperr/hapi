...
[Checking Questions](activity-checking) | How questions are automatically checked, and how their status is presented in elements | `checkElement`, `checkSection`, and `checkActivity` functions, `elementsFeedback` state
[Saving Progress](activity-saving-progress) | How activities save themselves to the local storage and to the drive | `localStorage`
[Activity Containers](activity-containers) | How activities are saved in small-sized files | iframes & cross-window communication, `postMessage`, `onMessage` event
